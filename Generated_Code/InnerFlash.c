...
} InnerFlash_TDeviceData;

typedef InnerFlash_TDeviceData *InnerFlash_TDeviceDataPtr; /* Pointer to the device data structure. */
/* {Default RTOS Adapter} Static object used for simulation of dynamic driver memory allocation */
static InnerFlash_TDeviceData DeviceDataPrv__DEFAULT_RTOS_ALLOC;
...
{
  InnerFlash_TDeviceDataPtr DeviceDataPrv;
  ...
  *(LDD_FLASH_TSafeRoutine *)(void *)DeviceDataPrv->SafeRoutinePtr = *(LDD_FLASH_TSafeRoutine *)(void *)&SafeRoutine; /* Copy the safe routine's code to a buffer in the internal data structure to be the code of the routine be runned from */
  ...
  PE_LDD_RegisterDeviceStructure(PE_LDD_COMPONENT_InnerFlash_ID,DeviceDataPrv);
...
{
  InnerFlash_TDeviceDataPtr DeviceDataPrv = (InnerFlash_TDeviceDataPtr)DeviceDataPtr; /* Auxiliary variable - pointer to an internal state structure */
...
LDD_TError InnerFlash_Erase(LDD_TDeviceData *DeviceDataPtr, LDD_FLASH_TAddress FromAddress, LDD_FLASH_TDataSize Size)
{
  ...
  if (((FromAddress & InnerFlash_ERASABLE_UNIT_MASK) != 0U) || ((Size & InnerFlash_ERASABLE_UNIT_MASK) != 0U)){ /* Is the desired area misaligned to erasable unit borders? */
    return ERR_PARAM_ADDRESS;          /* If yes, return error. */
  }
  /* Filling of the internal state structure */
  ...
  DeviceDataPrv->CurrentFlashAddress = FromAddress; /* Reset the "From address" for the operation */
  DeviceDataPrv->CurrentDataSize = InnerFlash_ERASABLE_UNIT_SIZE; /* Set the current data size */
...
void InnerFlash_Main(LDD_TDeviceData *DeviceDataPtr)
{
  ...
  if (DeviceDataPrv->CurrentOperationStatus == LDD_FLASH_START) { /* Is this run the first run of the Main method in this operation? */
    ...
    if (DeviceDataPrv->DataCounter == 0U) { /* If all the data has been successfully proceeded, finish the operation */
      ...
      InnerFlash_OnOperationComplete(DeviceDataPrv->UserDataPtr); /* Invoke the OnOperationComplete event */
      return;                          /* End */
    }
  }
  ...
  while ((StepsOfBatch > 0U) || (NextBatchCycle)) { /* Should another batch of the current operation be proceeded in this Main method processing? */
    ...
    switch(DeviceDataPrv->CurrentOperation) { /* Perform needed actions for the next step of the operation (for the next flash operation command) according to the current operation type */
      ...
      case LDD_FLASH_ERASE:            /* Erase operation */
        DeviceDataPrv->DataCounter -= (uint32_t)(InnerFlash_ERASABLE_UNIT_SIZE); /* Update of the Data counter */
        break;
...
static LDD_TError RangeCheck(LDD_FLASH_TAddress Address, LDD_FLASH_TDataSize Size)
{
  if ((Size > InnerFlash_PFLASH_SIZE) || (Size == 0U) || (Address > (InnerFlash_PFLASH_SIZE - Size))) {
    return ERR_PARAM_ADDRESS;
  }
  else {
    return ERR_OK;
